...
GPolygon::GPolygon(QVariantMap mp)
{
    info = mp;
    // 确保包含必要的属性
...
void GPolygon::loadFromJson(QJsonObject obj)
{
    info.clear();
    info["type"] = "hexagon";
...
void GPolygon::loadFromCoord(QVector2D st, QVector2D ed)
{
    info["type"] = "hexagon";
...
bool GPolygon::isInnerSpace(qreal x, qreal y)
{
    QVariantList points = info.value("points").toList();
...
void GPolygon::updatePosition(QVector2D relPos)
{
    QVariantList points = info.value("points").toList();
